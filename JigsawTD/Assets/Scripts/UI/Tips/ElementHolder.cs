...
public class ElementHolder : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void SetElementCount(StrategyBase strategy)
    {
        ...
        elementCountTxts[0].color = strategy.GoldCount > 0 ? Color.white : Color.gray;

        elementCountTxts[1].text = strategy.WoodCount.ToString();
        elementCountTxts[1].color = strategy.WoodCount > 0 ? Color.white : Color.gray;

        elementCountTxts[2].text = strategy.WaterCount.ToString();
        elementCountTxts[2].color = strategy.WaterCount > 0 ? Color.white : Color.gray;

        elementCountTxts[3].text = strategy.FireCount.ToString();
        elementCountTxts[3].color = strategy.FireCount > 0 ? Color.white : Color.gray;

        elementCountTxts[4].text = strategy.DustCount.ToString();
        elementCountTxts[4].color = strategy.DustCount > 0 ? Color.white : Color.gray;
